...
var handleRenderChart = function() {
  ...
  // small stat chart
	...
	$('[data-render="apexchart"]').each(function() {
		...
		if (chartType == 'bar') {
			...
			chartColors = [hexToRgba(COLOR_THEME, 1), hexToRgba(COLOR_THEME, .75), hexToRgba(COLOR_THEME, .5)];
			chartData = [randomNo(), randomNo(), randomNo()];
		} else if (chartType == 'donut') {
			chartColors = [hexToRgba(COLOR_THEME, .15), hexToRgba(COLOR_THEME, .35), hexToRgba(COLOR_THEME, .55), hexToRgba(COLOR_THEME, .75), hexToRgba(COLOR_THEME, .95)];
			chartData = [randomNo(), randomNo(), randomNo(), randomNo(), randomNo()];
			chartStroke = {
				show: false,
				curve: 'smooth',
				lineCap: 'butt',
				colors: hexToRgba(COLOR_BLACK, .25),
				width: 2,
				dashArray: 0,    
			};
			chartPlotOptions = {
  ...
  var serverChartOptions = {
    ...
    grid: {
    	show: true,
    	borderColor: hexToRgba(COLOR_WHITE, .15),
    },
    stroke: {
      show: false
    },
    colors: [hexToRgba(COLOR_WHITE, .25), COLOR_THEME],
    series: [{
    	name: 'MEMORY USAGE',
      data: [
