...
var handleRenderApexChart = function() {
	Apex = {
		...
		grid: {
			borderColor: hexToRgba(COLOR_WHITE, .25),
		},
		dataLabels: {
			style: {
				fontSize: '12px',
		...
		xaxis: {
			axisBorder: {
				show: true,
				color: hexToRgba(COLOR_WHITE, .25),
				height: 1,
				width: '100%',
				offsetX: 0,
				offsetY: -1
	...
	var apexLineChartOptions = {
		...
		colors: [COLOR_THEME, COLOR_GRAY_500],
		dataLabels: {
			enabled: true,
			background: {
				enabled: true,
		...
		grid: {
			row: {
				colors: [hexToRgba(COLOR_WHITE, .25), 'transparent'], // takes an array which will be repeated on columns
				opacity: 0.5
			}
		},
		series: [{
	...
	var apexColumnChartOptions = {
		...
		colors: [COLOR_GRAY_600, COLOR_INDIGO, COLOR_GRAY_300],
		series: [{
			name: 'Net Profit',
	...
	var apexAreaChartOptions = {
		...
		colors: [COLOR_PINK, COLOR_GRAY_600],
		series: [{
			name: 'series1',
	...
	var apexBarChartOptions = {
		...
		colors: [COLOR_ORANGE, COLOR_GRAY_500],
		stroke: {
			show: false
		},
		series: [{
	...
	var apexMixedChartOptions = {
		...
		colors: [COLOR_THEME, hexToRgba(COLOR_WHITE, .5), COLOR_ORANGE],
		title: {
	...
	var apexBubbleChartOptions = {
		...
		colors: [COLOR_THEME, COLOR_ORANGE, hexToRgba(COLOR_WHITE, .5), COLOR_PINK],
		series: [{
				name: 'Bubble1',
	...
	var apexScatterChartOptions = {
		...
		colors: [COLOR_THEME, COLOR_ORANGE, hexToRgba(COLOR_WHITE, .5)],
		series: [{
			name: "SAMPLE A",
	...
	var apexPieChartOptions = {
		...
		colors: [ hexToRgba(COLOR_PINK, .75),  hexToRgba(COLOR_ORANGE, .75),  hexToRgba(COLOR_THEME, .75), hexToRgba(COLOR_WHITE, .5),  hexToRgba(COLOR_INDIGO, .75)],
	...
	var apexRadialBarChartOptions = {
		...
		plotOptions: {
			radialBar: {
				...
				track: {
					background: hexToRgba(COLOR_WHITE, .25)
				},
				dataLabels: {
					name: {
						show: false,
		...
		colors: [COLOR_CYAN, COLOR_THEME, COLOR_INDIGO, COLOR_GRAY_300],
		series: [76, 67, 61, 90],
	...
	var apexRadarChartOptions = {
		...
		plotOptions: {
			radar: {
				size: 140,
				polygons: {
					strokeColors: hexToRgba(COLOR_WHITE, .25),
					strokeWidth: 1,
					connectorColors: hexToRgba(COLOR_WHITE, .25),
					fill: {
						colors: [hexToRgba(COLOR_GRAY_300, .25), hexToRgba(COLOR_WHITE, .25)]
					}
				}
			}
		},
