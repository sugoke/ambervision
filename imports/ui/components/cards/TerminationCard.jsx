...
export const TerminationCard = ({ report, product, characteristics }) => {
  const terminationData = report.terminationAnalysis || {};
  const payoffStructure = product?.payoffStructure || [];
  
  // Extract early termination levels from payoff structure
  const terminationLevels = [];
  const observationDates = product?.observationDates || [];
  
  // Find early termination components in the structure
  payoffStructure.forEach(item => {
    if ((item.type === 'autocall' || item.type === 'terminate') && item.value) {
      terminationLevels.push({
        level: parseFloat(item.value),
        label: item.label || 'Termination Level'
      });
    }
  });
        ...
        {terminationLevels.length > 0 && (
          <div style={{ marginBottom: '1rem' }}>
            <div style={{
              ...
              {characteristics.terminationType === 'stepDown' ? 'Step-Down Levels' : 'Termination Levels'}
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {terminationLevels.map((level, idx) => (
                <span key={idx} style={{
        ...
        {terminationData.probability && (
          <div style={{
            ...
            <div style={{
              ...
              Early Termination Probability: {(terminationData.probability * 100).toFixed(1)}%
            </div>
          </div>
        )}
      </div>
